...
> **עדכון (אותו יום):** ממצאים 1–6 תוקנו בענף זה. פירוט התיקון מופיע בתוך כל ממצא. מצב אחרי התיקונים: **242/242 טסטים עוברים** (נוספו 11 טסטים לאכיפת תוכנית), tsc נקי, ‏0 שגיאות lint, ה-build עובר וכל דפי ה-dashboard מרונדרים דינמית.
...
> ✅ **תוקן:** נוסף `requirePlanApi(role, route)` ב-`withRoleCheck.ts` שמחזיר 403 ורושם אירוע `plan_denied`. נאכף: `chat` + `coach/chats*` + `pattern-insights` + `weekly-report` ב-**deluxe**; `insights` ב-**pro** (עמוד היומן). `discovery` ו-`macro` נשארו לכל משתמש מחובר בכוונה — הם חלק מחוויית הדשבורד החינמית (DashboardView קורא להם ללא תנאי). מכוסה ב-11 טסטים חדשים (`tests/api/ai.planGate.test.ts`).
...
> ✅ **תוקן:** `await connection()` נוסף בראש `getUserContext()` (הדרך הנתמכת ב-Next 16 — ה-segment config‏ `dynamic` הוסר בגרסה זו). אומת ב-build: כל `/dashboard/*` עבר מ-`○ Static` ל-`ƒ Dynamic` גם כשה-build רץ בלי מפתחות.
...
> ✅ **תוקן:** `debug` הוסר מהתשובה לגמרי (גם ממסלול ההצלחה); האבחון נרשם עכשיו ללוג בצד שרת בלבד (`logger.info('insights empty', ...)`). גם פאנל ה-"temp debug info" הוסר מ-`AIInsightPanel.tsx`.
...
> ✅ **תוקן:** הסכימה דורשת כעת `^data:image\//` (כל mime של תמונה — כולל HEIC מאייפון, כדי לא לשבור שמירות קיימות). בנוסף `id` בסכימת הטרייד הוקשח ל-`.int()` ו-`/api/journal/[id]` מחזיר 400 על id לא-מספרי במקום 500.
...
> ✅ **תוקן:** הסדר הוחלף — `NEXT_PUBLIC_APP_URL` קודם, כותרת `Origin` רק כ-fallback לסביבות פיתוח. חובה להגדיר את המשתנה בפרודקשן (נוסף ל-`.env.example`).
...
> ✅ **תוקן (חלקית):** עודכנו `next@16.2.10`, ‏`eslint-config-next@16.2.10`, ‏`@clerk/nextjs@7.5.19` — כל הטסטים, tsc וה-build עברו אחרי העדכון. אזהרות ה-`postcss` הטרנזיטיביות נותרו (מצורפות בתוך next, בזמן build בלבד, אין fix זמין). הערה: ‏eslint-config-next החדש מפעיל חוקי React Compiler חדשים שסימנו ~20 דפוסים קיימים ב-UI — הורדו ל-warning ב-`eslint.config.mjs` עם הערה, לניקוי מסודר אחרי השחרור.
...
- [ ] לוודא שכל משתני הסביבה מוגדרים ב-Vercel (רשימה מלאה ב-`.env.example`), כולל `NEXT_PUBLIC_APP_URL`
- [ ] להגדיר את ה-webhooks בדשבורדים: Stripe → `/api/webhooks/stripe`, Clerk → `/api/webhooks/clerk`, ולהעתיק את ה-signing secrets
- [x] לתקן את ממצאים 1–3 (אכיפת plan ב-API, רינדור דינמי, הסרת debug) — ✅ בוצע
- [x] לעדכן `next` ו-`@clerk/nextjs` ולהריץ `npm test && npm run build` — ✅ בוצע (242/242)
- [ ] בדיקת עשן ידנית בפרודקשן: הרשמה → שדרוג בתשלום (מצב test של Stripe) → ביטול מנוי → לוודא ירידה ל-free
- [ ] אחרי השחרור: לנקות בהדרגה את אזהרות ה-React-Compiler החדשות (`react-hooks/set-state-in-effect` ועוד — ראו `eslint.config.mjs`)
