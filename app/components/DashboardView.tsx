...
const STR = {
  he: {
    nav: 'סקירה', idt: 'שעון ישראל', noSession: 'אין סשן פעיל',
    briefingK: 'התדריך היומי',
    zonePerf: 'ביצועים', zoneDecision: 'מרכז ההחלטות', zoneTools: 'כלים',

    // Signals
    sigNoTrades: 'עדיין לא תועדו עסקאות היום',
    sigTrades: (n: number) => `${n} עסקאות תועדו היום`,
    sigRulesKept: (n: number) => `${n} חוקים נשמרו היום`,
    sigRulesBroken: (n: number) => `${n} חוקים הופרו היום`,
    sigAiPattern: 'ה-AI זיהה דפוס חדש',
    sigMacro: (t: string, m: number) => `${t} בעוד ${m} דקות`,
    sigNoPlan: 'לא הוגדרה תוכנית להיום',
    ...
    // Focus + macro
    focusK: 'הפוקוס של היום', focusPh: 'על מה אתה מתמקד היום?',
    macroK: 'אירועי מאקרו היום', macroEmpty: 'אין אירועים בעלי השפעה גבוהה היום',
    macroUnavailable: 'לא ניתן לטעון את היומן הכלכלי כרגע',
  ...
  en: {
    nav: 'Overview', idt: 'ISRAEL TIME', noSession: 'NO ACTIVE SESSION',
    briefingK: 'DAILY BRIEFING',
    zonePerf: 'PERFORMANCE', zoneDecision: 'DECISION CENTER', zoneTools: 'TOOLS',

    sigNoTrades: 'No trades logged today yet',
    sigTrades: (n: number) => `${n} trades logged today`,
    sigRulesKept: (n: number) => `${n} rules kept today`,
    sigRulesBroken: (n: number) => `${n} rules broken today`,
    sigAiPattern: 'AI found a new pattern',
    sigMacro: (t: string, m: number) => `${t} in ${m} min`,
    sigNoPlan: 'No plan set for today',
    ...
    focusK: "TODAY'S FOCUS", focusPh: 'What are you focusing on today?',
    macroK: 'MACRO EVENTS TODAY', macroEmpty: 'No high-impact events today',
    macroUnavailable: 'Economic calendar unavailable right now',
...
export default function DashboardView() {
  ...
  const minsToMacro = nextMacro ? hhmmToMin(nextMacro.timeIsrael) - nowMin : null;

  /* ── Live signals — the dashboard's pulse. Real data only. ────── */
  const signals: { tone: 'gold' | 'bull' | 'bear' | 'muted'; text: string }[] = [];
  if (tradesToday.length === 0) signals.push({ tone: 'muted', text: s.sigNoTrades });
  else signals.push({ tone: 'gold', text: s.sigTrades(tradesToday.length) });
  if (ruleStats.today.followed > 0) signals.push({ tone: 'bull', text: s.sigRulesKept(ruleStats.today.followed) });
  if (ruleStats.today.violated > 0) signals.push({ tone: 'bear', text: s.sigRulesBroken(ruleStats.today.violated) });
  if (discovery) signals.push({ tone: 'gold', text: s.sigAiPattern });
  if (nextMacro && minsToMacro != null && minsToMacro <= 240) signals.push({ tone: 'gold', text: s.sigMacro(nextMacro.title, minsToMacro) });
  if (!saved.bias) signals.push({ tone: 'muted', text: s.sigNoPlan });
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, minWidth: 0, overflowY: 'auto' }}>
        <div className="dp-cmd">
          ...
          {/* ════ 1 · DAILY BRIEFING ════ */}
          <section className="dp-brief-bar dp-reveal dp-rev-1">
            <div className="dp-brief-bar-main">
              <span className="dp-brief-kicker">{s.briefingK}</span>
              <h1 className="dp-brief-heading">{dateStr}</h1>
            ...
            <div className="dp-signals">
              {signals.map(sig => (
                <span key={sig.text} className={`dp-signal dp-signal-${sig.tone}`}>{sig.text}</span>
              ))}
            </div>
          </section>

          {/* ════ 2 · THREE ZONES ════ */}
          <div className="dp-zones">

            {/* ── LEFT · PERFORMANCE ── */}
            <section className="dp-zone dp-reveal dp-rev-2">
              <span className="dp-zone-title">{s.zonePerf}</span>

              <div className="panel dp-pad">
                <div className="dp-stat-row">
                  <span className="dp-stat-label">{s.pnlToday}</span>
                  <span className="dp-stat-value" style={{ color: sToday.n === 0 ? 'var(--w40)' : sToday.pnl >= 0 ? 'var(--bull)' : 'var(--bear)' }}>
                    {sToday.n === 0 ? '—' : money(sToday.pnl)}
                  </span>
                  <span className="dp-stat-story">{sToday.n === 0 ? s.sigNoTrades : s.basedOn(sToday.decided)}</span>
                </div>
                <div className="dp-stat-sep" />
                <div className="dp-stat-row">
                  <span className="dp-stat-label">{s.pnlWeek}</span>
                  <span className="dp-stat-value" style={{ color: sWeek.n === 0 ? 'var(--w40)' : sWeek.pnl >= 0 ? 'var(--bull)' : 'var(--bear)' }}>
                ...
                <div className="dp-stat-row">
                  ...
                  <span className="dp-stat-value" style={{ color: sMonth.n === 0 ? 'var(--w40)' : sMonth.pnl >= 0 ? 'var(--bull)' : 'var(--bear)' }}>
              ...
              <div className="panel dp-pad">
                <div className="dp-stat-row">
                  ...
                  <span className="dp-stat-value">{fmtPct(sMonth.wr)}</span>
                  <span className="dp-stat-story">{winStory}</span>
                </div>
                <div className="dp-stat-sep" />
                <div className="dp-stat-row">
                  <span className="dp-stat-label">{s.avgR}</span>
                  <span className="dp-stat-value" style={{ color: (sMonth.avgR ?? 0) >= 0 ? 'var(--bull)' : 'var(--bear)' }}>{fmtR(sMonth.avgR)}</span>
                  <span className="dp-stat-story">{avgRStory}</span>
                </div>
                <div className="dp-stat-sep" />
                <div className="dp-stat-row">
                  <span className="dp-stat-label">{s.profitFactor}</span>
                  <span className="dp-stat-value">{fmtPf(sMonth.pf)}</span>
            ...
            {/* ── CENTER · DECISION ── */}
            <section className="dp-zone dp-zone-center dp-reveal dp-rev-2">
              <span className="dp-zone-title">{s.zoneDecision}</span>

              {/* AI INSIGHT OF THE DAY — the focal point */}
              <div className="dp-ai-hero">
              ...
              {/* FOCUS + MACRO */}
              <div className="dp-center-duo">
                <div className="panel dp-pad">
                  <span className="dp-zone-title">{s.focusK}</span>
                  <input className="dp-field-input" style={{ marginTop: 12 }} value={focus} onChange={e => setFocus(e.target.value)} onBlur={handleSaveFocus} placeholder={s.focusPh} />
                  {focusSaved && <span className="dp-mission-empty" style={{ color: 'var(--bull)' }}>{s.savedOk} ✓</span>}
                </div>
                  ...
                  <div className="dp-macro-list">
                    {macro == null ? (
                      ...
                      primaryMacro.slice(0, 5).map((e, i) => (
                        <div key={`${e.title}-${i}`} className="dp-macro-row">
            ...
            {/* ── RIGHT · TOOLS ── */}
            <section className="dp-zone dp-reveal dp-rev-3">
              <span className="dp-zone-title">{s.zoneTools}</span>

              <div className="panel dp-pad dp-quick">
                <Link href="/dashboard/journal" className="dp-cta-primary">{s.ctaNewTrade}</Link>
                <Link href="/dashboard/journal" className="dp-cta-ghost">{s.ctaJournal}</Link>
                <Link href="/dashboard/coach" className="dp-cta-ghost">{s.ctaCoach}</Link>
              </div>

              {/* Risk calculator — accessible, deliberately quiet */}
              <div className="panel dp-pad">
                <span className="dp-zone-title">{s.calcK}</span>
                <div className="dp-seg-grid-2" style={{ marginTop: 12 }}>
