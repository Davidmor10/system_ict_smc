...
describe('generateNarrativeText', () => {
  ...
  it('never asks for the old fixed-template field names in the prompt', async () => {
    generateInsightJsonMock.mockResolvedValue(JSON.stringify({ paragraphs: ['a', 'b', 'c'] }));
    await generateNarrativeText(baseFacts(), 'he');
    const prompt = generateInsightJsonMock.mock.calls[0][0] as string;
  ...
  it('includes the challenge-the-trader rule only when notes are present', async () => {
    ...
    expect(generateInsightJsonMock.mock.calls[0][0]).not.toContain('OBJECTIVITY');

    await generateNarrativeText(baseFacts({ notesObservations: ['הפסדתי בגלל החדשות'] }), 'he');
    expect(generateInsightJsonMock.mock.calls[1][0]).toContain('OBJECTIVITY');
    expect(generateInsightJsonMock.mock.calls[1][0]).toContain('הפסדתי בגלל החדשות');
  });

  it('parses a well-formed response into paragraphs', async () => {
    generateInsightJsonMock.mockResolvedValue(JSON.stringify({ paragraphs: ['פסקה ראשונה', 'פסקה שנייה', 'פסקה שלישית'] }));
  ...
  it('returns null when the model returns fewer than 3 paragraphs', async () => {
    generateInsightJsonMock.mockResolvedValue(JSON.stringify({ paragraphs: ['only one'] }));
  ...
  it('caps paragraphs at 6 even if the model returns more', async () => {
    generateInsightJsonMock.mockResolvedValue(JSON.stringify({ paragraphs: ['1', '2', '3', '4', '5', '6', '7', '8'] }));
    const result = await generateNarrativeText(baseFacts(), 'he');
